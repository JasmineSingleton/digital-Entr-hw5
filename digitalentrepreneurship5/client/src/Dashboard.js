...
function Dashboard({ name, email, onLogout }) {
  ...
  useEffect(() => {
    const savedMode = localStorage.getItem('pulseDarkMode');
    if (savedMode) setDarkMode(JSON.parse(savedMode));
  }, []);

  useEffect(() => {
    localStorage.setItem('pulseDarkMode', JSON.stringify(darkMode));
  }, [darkMode]);
    ...
    return (
      <div className="review-card">
        ...
        <div className="review-ratings">
          <div className="rating-item"><span className="rating-label">Teaching:</span><span className="rating-value">{ratings.teaching}</span></div>
          <div className="rating-item"><span className="rating-label">Difficulty:</span><span className="rating-value">{ratings.difficulty}</span></div>
          <div className="rating-item"><span className="rating-label">Organization:</span><span className="rating-value">{ratings.organization}</span></div>
          <div className="rating-item"><span className="rating-label">Helpfulness:</span><span className="rating-value">{ratings.helpfulness}</span></div>
        </div>
  ...
  const renderSubmitTab = () => (
    <div className="submit-tab">
      ...
      <div className="guidelines-box">
        ...
        <p>Please keep your review respectful and constructive...</p>
      ...
      <form className="review-form" onSubmit={handleSubmitReview}>
        ...
        <div className="form-group ratings-group">
          <label>Ratings</label>
          <div className="rating-row"><span className="rating-label">Teaching:</span><StarRating value={teachingRating} onChange={setTeachingRating} /></div>
          <div className="rating-row"><span className="rating-label">Difficulty:</span><StarRating value={difficultyRating} onChange={setDifficultyRating} /></div>
          <div className="rating-row"><span className="rating-label">Organization:</span><StarRating value={organizationRating} onChange={setOrganizationRating} /></div>
          <div className="rating-row"><span className="rating-label">Helpfulness:</span><StarRating value={helpfulnessRating} onChange={setHelpfulnessRating} /></div>
          <div className="rating-row"><span className="rating-label">Overall Rating:</span><StarRating value={overallRating} onChange={setOverallRating} /></div>
        </div>

        <div className="form-group">
          <label>Your Review*</label>
          <textarea value={reviewText} onChange={(e) => setReviewText(e.target.value)} rows={6} required />
        </div>
    ...
    <div className="my-reviews-tab">
      ...
      {loading ? (
        ...
        <div className="my-reviews-list">
          {reviews.filter(r => r.studentEmail === email).length > 0 ? (
            reviews.filter(r => r.studentEmail === email).map(r => <ReviewCard key={r.id} review={r} />)
          ) : (
  ...
  return (
    <div className={`dashboard ${darkMode ? 'dark-mode' : ''}`}>
      ...
      <nav className="dashboard-nav">
        <button className={`tab-btn ${activeTab === 'search' ? 'active' : ''}`} onClick={() => setActiveTab('search')}>Browse Reviews</button>
        <button className={`tab-btn ${activeTab === 'submit' ? 'active' : ''}`} onClick={() => setActiveTab('submit')}>Submit Review</button>
        <button className={`tab-btn ${activeTab === 'myReviews' ? 'active' : ''}`} onClick={() => setActiveTab('myReviews')}>My Reviews</button>
      </nav>
